...
| PostgreSQL storage migration | PARTIAL | SQL schema/migration runner exist under `services/control-api/migrations/` and `services/control-api/src/run-migrations.mjs`; `STORE_DRIVER` defaults to JSON through the `store.mjs` selector, while `store-postgres.mjs` implements the same read/update/write/audit interface with transactional read-mutate-write semantics. JSON-mode `npm test` passes. Local Postgres migration and API smoke verification are blocked in this environment because Docker/PostgreSQL is not installed. |
