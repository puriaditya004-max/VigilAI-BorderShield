...
| Offline edge operation and synchronization | PARTIAL | Durable JSON outbox and replay in `edge/edge-agent/src/outbox.mjs`; bridge startup no longer crashes if camera registration is unavailable and queues incidents until the command link recovers. Camera registration retries use exponential backoff starting at 2s and capped at 60s, while outbox replay retries start at 2s and cap at 30s so field producers cannot self-trigger API rate-limit lockout. Covered by `tests/integration/offline-startup-queue.mjs` and `tests/integration/registration-backoff.mjs`. Evidence encryption is optional, bounded rolling frame buffer foundation exists, encrypted rolling video buffer pending. |
