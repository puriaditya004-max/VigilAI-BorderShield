...
| Offline edge operation and synchronization | PARTIAL | Durable JSON outbox and replay in `edge/edge-agent/src/outbox.mjs`; bridge startup no longer crashes if camera registration is unavailable and queues incidents until the command link recovers. Camera registration retries use exponential backoff starting at 2s and capped at 60s, while outbox replay retries start at 2s and cap at 30s so field producers cannot self-trigger API rate-limit lockout. Incident IDs include camera, type, track/frame, capture timestamp and a monotonic suffix so same-millisecond crossings cannot overwrite queued outbox files. Covered by `tests/integration/offline-startup-queue.mjs`, `tests/integration/registration-backoff.mjs` and `tests/integration/same-millisecond-incident-ids.mjs`. Evidence encryption is optional, bounded rolling frame buffer foundation exists, encrypted rolling video buffer pending. |
...
| PostgreSQL storage migration | PARTIAL | SQL schema/migration runner exist under `services/control-api/migrations/` and `services/control-api/src/run-migrations.mjs`; `STORE_DRIVER` defaults to JSON through the `store.mjs` selector, while `store-postgres.mjs` implements the same read/update/write/audit interface with transactional read-mutate-write semantics. JSON-mode `npm test` passes. `npm run postgres:test` provides a fresh-schema migration/API smoke test when `POSTGRES_URL` or `DATABASE_URL` is available; it is blocked in this environment because Docker/PostgreSQL is not installed/configured. |
